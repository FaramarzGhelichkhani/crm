...
from Emdad.models import Service, Motor
exit()
...
# Technecian.objects.bulk_create([Technecian(**data) for data in tech])
...
# Service.objects.bulk_create([Service(**{'name':data}) for data in serviceslist])
...
# leadcsvreader = list(leadcsvreader)
# for lead in leadcsvreader:
#     extracted_data = {ordermap[field]: lead[field] for field in lead.keys() if field in ordermap}
#     extracted_data['technecian'] = Technecian.objects.get(pk=lead['agent_id']) if lead['agent_id'] != '' else None
...
for row in transactionscsvreader:
    if row['category'] == 'واریزی توسط تکنسین':
        trans_data = {
            ...
            'technician':Technecian.objects.get(pk=row['technician_id'])
        }
