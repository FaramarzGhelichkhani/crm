...
class AgentDetailView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_sucess_lead_number = Order.objects.filter(
            technecian__id=self.kwargs["pk"], status="انجام شد").count()
        grade_avg = Order.objects.filter(
            technecian__id=self.kwargs["pk"], status="انجام شد").aggregate(Avg('grade'))
        ...
        total_wage = leads.aggregate(Order_wage__sum=Sum('wage'))[
            "Order_wage__sum"]
        total_commisions = leads.filter(status="انجام شد").aggregate(
